(function(global) {
  ...
  function Pig(imageData, options) {
    ...
    this.settings = {
      ...
      getImageSize: function(lastWindowWidth) {
        if (lastWindowWidth <= 640) return 100;
        else if (lastWindowWidth <= 1920) return 250;
        return 500;
      }
    };
  ...
  Pig.prototype._recomputeMinAspectRatio = function() {
    ...
    else this.minAspectRatioRequiresTransition = false;
  };
  ...
  Pig.prototype._doLayout = function() {
    ...
    this.images.forEach(
      function(image) {
        if (
          image.style.translateY + image.style.height <
    ...
    setTimeout(
      function() {
        ...
        if (!this.thumbnail) {
          ...
          this.thumbnail.src = this.pig.settings.urlForSize(
            this.filename,
